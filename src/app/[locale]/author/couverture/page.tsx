...
import { Image as ImageIcon, CheckCircle2, Gavel } from "lucide-react";
...
export default async function CouverturePage({
  ...
  const covers = (await getDossierValidations(active.id)).filter((v) => v.kind === "COVER");
  const pending = covers.filter((v) => v.status === "PENDING" && !v.expired);
  const past = covers.filter((v) => !(v.status === "PENDING" && !v.expired));
  ...
  return (
    <div className="space-y-6">
      ...
      {past.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>{tv("pastTitle")}</CardTitle>
          </CardHeader>
